...
public struct LiftCalculator
{
    ...
    {
        return polygon.Edges.Sum(t => CalculateLiftCoefficientOfEdge(t, velocity));
    }
    ...
    private static float CalculateLiftCoefficientOfEdge(Edge edge, Vector3 velocity)
    {
        ...
        var liftCoefficient = 2 * velocityNormal * velocityTangent /
                              (velocityNormal * velocityNormal + velocityTangent * velocityTangent);
        return liftCoefficient;
    }
}