...
public class Board<T> : IObservable<T>, ICloneable
{
 ...
 public static BoardHistory<T> History { get; } = new BoardHistory<T>();
 ...
 public T[] GetRow(int row)
 {
     ...
     {
         rowArray[i] = _board[row, i]; // Here, use the row as is, because we are assuming index starts from 0
     }
     return rowArray;
 }
 public T[] GetColumn(int column)
 {
     ...
     for (var i = 0; i < _board.GetLength(0); i++)
     {
         columnArray[i] = _board[i, column]; // Here, use the column as is, because we are assuming index starts from 0
     }
     return columnArray;
 }
