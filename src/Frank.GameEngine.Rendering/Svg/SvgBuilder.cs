...
public class SvgBuilder
{
    ...
    public SvgBuilder(SvgRendererOptions options, Rectangle viewBox)
    {
        ...
        switch (_options.Style)
        {
            case SvgStyle.Normal:
                ...
                break;
            case SvgStyle.Html:
                _svgBuilder.AppendLine(
                    $"<svg viewBox='{_viewBox.ToSvgViewBox()}' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'>");
                break;
    ...
    public SvgBuilder WithGridLines(int gridLineSpacing = 10)
    {
        ...
        for (var x = _viewBox.X; x < _viewBox.Width; x += gridLineSpacing)
        {
            var strokeOpacity = x == 0
                ...
                : "0.25";
            var line =
                $"{Indent(1)}<line x1='{x}' y1='{_viewBox.Y}' x2='{x}' y2='{_viewBox.Height}' stroke='gray' stroke-width='1' stroke-opacity='{strokeOpacity}' />";
        ...
        for (var y = _viewBox.Y; y < _viewBox.Height; y += gridLineSpacing)
        {
            var strokeOpacity = y == 0
                ...
                : "0.25";
            var line =
                $"{Indent(1)}<line x1='{_viewBox.X}' y1='{y}' x2='{_viewBox.Width}' y2='{y}' stroke='gray' stroke-width='1' stroke-opacity='{strokeOpacity}' />";
            _svgBuilder.AppendLine(line);
        }
    ...
    public SvgBuilder WithLegend()
    {
        ...
        _svgBuilder.AppendLine(
            $"{Indent(1)}<line x1='0' y1='0' x2='{textOffset}' y2='0' stroke='black' stroke-width='1' />");
        _svgBuilder.AppendLine($"{Indent(1)}<text x='{length}' y='0' font-size='{fontSize}'>X</text>");
        _svgBuilder.AppendLine(
            $"{Indent(1)}<line x1='0' y1='0' x2='0' y2='{textOffset}' stroke='black' stroke-width='1' />");
    ...
    {
        return new string(' ', IndentSize * indentLevel);
    }
}